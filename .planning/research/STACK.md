...
**Domain:** Music archive + analytics web app (personal, 4 users, dual platform import)
**Researched:** 2026-06-11
**Confidence:** HIGH (core choices), MEDIUM (Apple Music integration)
...
Better Auth's Spotify social provider handles the PKCE flow automatically. You declare the required scopes explicitly:
...
The `playlist-read-private` scope is required to read playlists owned by the authenticated user. Without it, `GET /playlists/{id}/items` returns only metadata.

### February 2026 API Changes — CRITICAL

Spotify's February 2026 Dev Mode changes affect this app:
...
**Implication for this app**: Since all playlists are owned by one of the four authenticated users, the owned-playlist restriction is not a blocker. The app owner must keep Premium active. The 4-user ceiling comfortably fits the group.
...
Apple Music uses a **two-token model**, not standard OAuth:

1. **Developer Token (JWT)**: Server-side only. Sign using ES256 with your Apple Developer Team ID, MusicKit Key ID, and a `.p8` private key file. Use the `jose` library in a Next.js API route (`/api/apple/token`). Token lifetime up to 6 months; regenerate and re-deploy periodically.

2. **Music User Token**: Browser-side only. After loading MusicKit JS v3 from Apple's CDN, call `MusicKit.getInstance().authorize()` which opens a popup. This returns the user's music token and must be stored (in Better Auth's session or a separate DB column) for subsequent API calls.
...
Load via a Next.js `<Script>` component with `strategy="lazyOnload"` to avoid blocking the initial render.

### Apple Music Playlist Fetch

After authorization, use the MusicKit API:
...
Paginate with `?limit=100&offset=N`. For playlist tracks:

```
GET https://api.music.apple.com/v1/me/library/playlists/{id}/tracks
```

**Known issues**: Some developers report 403 errors on `/me/library/playlists` despite valid tokens. This appears to be a MusicKit entitlement issue — the Apple Developer account must have MusicKit enabled in the Certificates, Identifiers & Profiles section.

### Apple Developer Program Cost

$99 USD/year (approx. $150 AUD/year) — required for the MusicKit private key. This is unavoidable.

---

## Database Schema Overview

The Drizzle schema will need these main tables:
...
Keep the schema flat. No EAV, no JSON blobs for core analytics fields.
...
```bash
# Scaffold
pnpm create next-app@latest warwick-massive-tunage --typescript --tailwind --app --turbopack
...
pnpm dlx shadcn@latest add card chart table badge
...
**Import job (Spotify/Apple Music sync):**
...
*Stack research for: Warwick Massive Tunage — music archive + analytics web app*
*Researched: 2026-06-11*
